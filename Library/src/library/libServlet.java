...
public class libServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(operation.equals("add"))
		{
			...
			String query="insert into student(Name,class,Listofcards,Expirydate)values('"+Name+"','"+std+"','"+List+"','"+date+"')";
			stat.execute(query);
	   ...
	   else if(operation.equals("update"))
	{
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/student", "root", "root");
			Statement stat = con.createStatement();
			String query="update student set Name='"+Name+"',class='"+std+"',Listofcards='"+List+"',Expirydate='"+date+"'where Rollno="+Rollno;
			stat.execute(query);
